...
class Rotate15:
    '''
    Rotates images counter-clockwise by 15 degree.
    '''
    ...
    def __call__(self, image, labels=None): 
        ...
        for i in range(n):
            b = BoundingBox(x1 = labels[i, xmin], y1 = labels[i, ymin], 
                             x2 = labels[i, xmax], y2 = labels[i, ymax])
            bb.append(b)
         
        bbs = BoundingBoxesOnImage(bb, shape = image.shape)   
        seq = iaa.Affine(rotate = self.angle) 
        ...
        if labels is None:
            ...
            for i in range(len(bbs.bounding_boxes)):
                ...
                labels[i, xmin] = after.x1
                labels[i, ymin] = after.y1
                labels[i, xmax] = after.x2
                labels[i, ymax] = after.y2
...
class Padding:
    ...
    def __init__(self, translate_px = 0, translate_py = 0,
                 labels_format={'class_id': 0, 'xmin': 1, 'ymin': 2, 'xmax': 3, 'ymax': 4}):
        '''
        Arguments:
            mul (number or tuple of number or list of number or imgaug.parameters.StochasticParameter, optional): 
        ...
        self.translate_py = translate_py
        self.labels_format = labels_format
    ...
    def __call__(self, image, labels=None): 
        ...
        bbs = BoundingBoxesOnImage(bb, shape = image.shape)   
        seq = iaa.Affine(translate_px = {'x': self.translate_px, 'y': self.translate_py})
        image_aug, bbs_aug = seq(image=image, bounding_boxes=bbs)  
