...
from data_generator.object_detection_2d_geometric_ops import ResizeRandomInterp, RandomFlip, Rotate15, Multiply, Padding
...
class SSDDataAugmentation:
    ...
    def __init__(self,
        ...
        self.rotate15 = Rotate15(angle=10, labels_format=self.labels_format)
        self.multiply = Multiply(mul=(0.8, 1.2), labels_format=self.labels_format)
        self.padding = Padding(translate_px=100, translate_py=100)
        ...
        self.sequence1 = [self.photometric_distortions,
                          self.expand,
                          self.random_crop,
                          self.resize]
        ...
        self.sequence3 = [self.photometric_distortions,
                            self.resize]
        self.sequence4 = [self.expand,
                            self.resize]
        # self.sequence5 = [self.padding,
        #                     self.resize]

        self.sequence5 = [self.rotate15,
                        self.resize]
        self.sequence6 = [self.random_crop,
                            self.resize]
        self.sequence7 = [self.resize]

        self.sequence = [self.sequence1, self.sequence2, self.sequence3, self.sequence4, self.sequence5, self.sequence6, self.sequence7]
    ...
    def __call__(self, image, labels, return_inverter=False):
        ...
        inverters = [] 
        # Choose sequence 1 with probability 0.5.
        n = np.random.choice(7)
        for transform in self.sequence[n]: 
