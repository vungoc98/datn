...
#         # print('len dataaaaaaaaaaaaaaaaa: ', len(data))
...
#         self.dataset_size = len(self.filenames)
#         # print('len dataset_size: ', len(self.filenames))
#         self.dataset_indices = np.arange(self.dataset_size, dtype=np.int32)
#         if self.load_images_into_memory:
#             self.images = []
#             if verbose: it = tqdm(self.filenames, desc='Loading images into memory', file=sys.stdout)
...
#         print('len dataaaaaaaaa: ', len(self.filenames))
#         self.dataset_size = len(self.filenames)
#         self.dataset_indices = np.arange(self.dataset_size, dtype=np.int32)
#         if self.load_images_into_memory:
#             self.images = []
...
class DataGenerator:
    ...
    def parse_json(self,
        ...
        self.cats_to_classes = {} # A dictionary that maps between the original (keys) and the transformed IDs (values)
        self.classes_to_cats = {} # A dictionary that maps between the transformed (keys) and the original IDs (values) 
        for i, cat in enumerate(annotations['types']):
            i = i + 1
            self.cats_to_names[i] = cat
            self.classes_to_names.append(cat)
            self.cats_to_classes[i] = i + 1
            self.classes_to_cats[i + 1] = i

        # print(self.classes_to_cats)
        # print(self.classes_to_cats[220])
       

        # Iterate over all datasets.
        for images_dir, annotations_filename in zip(self.images_dirs, self.annotations_filenames):
            # print('imagesdir: ', images_dir)
            ...
            if ground_truth_available:
                ...
                # and whose values are the annotations for the respective image ID.
                image_ids_to_annotations = defaultdict(list)
                for annotation in annotations['imgs'].values():
                    image_ids_to_annotations[annotation['id']].append(annotation['objects'])

            if verbose: it = tqdm(annotations['imgs'].values(), desc="Processing '{}'".format(os.path.basename(annotations_filename)), file=sys.stdout)
            else: it = annotations['images']
            ...
            for img in it:  
                path = img['path']
                path = path.split('/')
                if path[0] == mode and os.path.exists(os.path.join(images_dir, img['path'])): 
                    self.filenames.append(os.path.join(images_dir, img['path']))
                    self.image_ids.append(img['id'])
                    # print(img['id'], path)
                    ...
                    if ground_truth_available:
                        ...
                        annotations = image_ids_to_annotations[img['id']] 
                        boxes = []
                        for objects in annotations: 
                            if len(objects) > 0:
                                for annotation in objects: 
                                    # print(self.classes_to_names) 
                                    cat_id = self.classes_to_names.index(annotation['category'])
                                    # Check if this class is supposed to be included in the dataset.
                                    if (not self.include_classes == 'all') and (not cat_id in self.include_classes): continue
                                    # Transform the original class ID to fit in the sequence of consecutive IDs.
                                    # print(img['id']) 
                                    class_id = self.cats_to_classes[cat_id]
                                    # print(cat_id, class_id)
                                    # print(img['id'], class_id)
                                    xmin = annotation['bbox']['xmin']
                                    ymin = annotation['bbox']['ymin']
                                    xmax = annotation['bbox']['xmax']
                                    ymax = annotation['bbox']['ymax'] 
                                    item_dict = {'image_name': img['path'],
                                                'image_id': img['id'],
                                                'class_id': class_id,
                                                'xmin': xmin,
                                                'ymin': ymin,
                                    ...
                                    boxes.append(box)
                        self.labels.append(boxes)
        # print('self labelsssssssss: ', self.labels)
    ...
    def generate(self,
        ...
        while True:
            ...
            for i in range(len(batch_X)):
                ...
                # tim lai bounding box cua tung tile
                labels = list(batch_y[i])
                ...
                for k in range(len(imgs)):
                    ...
                    c_x -= 1 
                    for gt in labels:
                        if self.checkBB(gt, input_size[k], imgs[k].shape, stride):
                            class_id, xmin, ymin, xmax, ymax = self.findBB(gt, input_size[k], stride)
                            tile_bbs.append([class_id, xmin, ymin, xmax, ymax])
                    # if len(tile_bbs) != 0:
                    #     tile_bbs = np.array(tile_bbs)
                    #     batch_X.append(imgs[k])
                    #     batch_y.append(tile_bbs)
                    #     batch_filenames.append(batch_filenames[i])
                    #     batch_image_ids.append(batch_image_ids[i]) 

                    batch_y.append(np.array(tile_bbs))
                # bo sung filename khi spliting
                batch_filenames += batch_filenames[i] * len(imgs)
                batch_image_ids += batch_image_ids[i] * len(imgs)
                batch_X += imgs
            ...
            # # bo sung splitting  
            # # bo sung filename khi spliting
            # batch_filenames += [0] * 4 * len(batch_filenames)
            # for i in range(len(batch_X)):
            #     img = batch_X[i]
            #     h, w, _ = img.shape
            #     H = h // 2
            #     W = w // 2
            #     tiles = [img[x:x+H,y:y+W] for x in range(0,h,H) for y in range(0,w,W)] 
            #     tile_bbs = []
            #     bbs = list(batch_y[i])
            ...
            #         # bb thuoc tile 1
            #         if xmin < W and ymin < H and xmax < W and ymax < H:  
            #             xmint1, ymint1, xmaxt1, ymaxt1 = xmin, ymin, xmax, ymax
            #             tile_bbs[0].append([class_id, xmint1, ymint1, xmaxt1, ymaxt1])

            #         # bb thuoc tile 2:
            #         elif xmin >= W and ymin < H and ymax < H:
            #             xmint1 = xmin - W
            #             ymint1 = ymin
            #             xmaxt1 = xmax - W
            #             ymaxt1 = ymax
            #             tile_bbs[1].append([class_id, xmint1, ymint1, xmaxt1, ymaxt1])
                    
            #         # bb thuoc tile 3
            #         elif xmin < W and ymin >= H and xmax < W:
            #             xmint1 = xmin
            #             ymint1 = ymin - H
            #             xmaxt1 = xmax
            #             ymaxt1 = ymax - H
            #             tile_bbs[2].append([class_id, xmint1, ymint1, xmaxt1, ymaxt1])
                    
            #         ## bb thuoc tile 4
            #         elif xmin >= W and ymin >= H:
            #             xmint1 = xmin - W
            #             ymint1 = ymin - H
            #             xmaxt1 = xmax - W
            #             ymaxt1 = ymax - H
            #             tile_bbs[3].append([class_id, xmint1, ymint1, xmaxt1, ymaxt1])

            #     batch_X += tiles
            #     for j in range(len(tile_bbs)): 
            #         # if len(tile_bbs[j]) > 0:
            #         batch_y.append(np.array(tile_bbs[j]))
            #         # batch_X.append(tiles[j])
            #         # batch_image_ids.append(batch_image_ids[i])
            ...
            # # add ids for splitting image
            # for i in range(len(batch_image_ids)):
            #     batch_image_ids += [batch_image_ids[i]] * 4
    ...
    def checkBB(self, gt, tile, size, stride):
    ...
    def findBB(self, gt, tile, stride):
        class_id, xmin, ymin, xmax, ymax = gt
        x, y, c_y, c_x= tile
