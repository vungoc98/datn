...
model.load_weights(os.path.join(fileDir, 'mobilenet_v2_weights_tf_dim_ordering_tf_kernels_1.0_224.h5'), by_name=True,skip_mismatch=True)
# model.load_weights(os.path.join(fileDir, 'training_mobilenetv2ssd512_new_scales_splitting_image_epoch-101_loss-5.1552_val_loss-7.9082.h5'), by_name=True, skip_mismatch=True)
...
train_image_dir = os.path.join(rootDir, 'train_full')
val_image_dir = os.path.join(rootDir, 'test_full')
train_labels_filename = os.path.join(fileDir, 'train.csv')
val_labels_filename   = os.path.join(fileDir, 'test.csv')
images_t, filenames_t, labels_t, image_ids_t = train_dataset.parse_csv(images_dir=train_image_dir,
...
# print(len(filenames_t))

# print(len(labels_t))

val_dataset.parse_csv(images_dir=val_image_dir,
...
model_checkpoint = ModelCheckpoint(filepath='training_mobilenetv2ssd512_scales_for_splitting_image_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
                                   monitor='loss',
                                   verbose=1,
                                   save_best_only=True,
...
csv_logger = CSVLogger("training_mobilenetv2ssd512_scales_for_splitting_image.log")
...
### class weight
# class_weight imbalance class
n = 43 
y_train = []
for labels in labels_t:   
    y_train += list(labels[:, 0])

class_weights_from_sklearn = class_weight.compute_class_weight('balanced', np.unique(y_train), y_train) 
 
class_weights_from_sklearn = [0] + list(class_weights_from_sklearn)

class_weights = np.zeros((6132, 56))

for i in range(1, 44): 
    class_weights[:, i] += class_weights_from_sklearn[i]
