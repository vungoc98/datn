...
import math
import operator # sorted array in python
...
class Evaluator:
    ...
    def predict_on_dataset(self,
        ...
        # lay chi so class_id, conf, xmin, ymin, xmax, ymax (0, 1, 2, 3, 4) dua vao dictionary cua class
        ...
        n_batches = int(ceil(n_images / batch_size)) # lap n_batches
        if verbose:
        ...
        for j in tr:
            # Generate batch.
            batch_X, batch_image_ids, batch_eval_neutral, batch_inverse_transforms, batch_orig_images, batch_orig_labels = next(generator) # moi lan lay batch_size hinh anh

            # thuc hien split tren tung batch_X
            #### addd
            print('------------------------------batch_X-------------------------------: ', batch_X.shape)
            print('------------------------------batch orig images--------------------------: ', len(batch_orig_images))
            # print(batch_orig_images[0].shape) 
            
            ## start thuc hien splitting image into tiles
            input_images = []
            orig_images = []
            batch = []
            y_pred = []
            ...
            for i in range(len(batch_X)): 
                input_image = [] 
                image = batch_orig_images[i] # lay anh goc
                H, W, _ = image.shape
                stride = 400
                size = 512
                imgs = []
                ...
                imgs_predicted = self.model.predict(input_image)
        ...
        #         input_image = []
        #         orig_image = []
        #         h, w, _ = batch_orig_images[i].shape
        #         h_o, w_o = h, w
        #         H = h // 2
        #         W = w // 2
        #         orig_img = batch_orig_images[i] 
        ...
        #         # y_pred_filtered = sorted(y_pred_filtered, key=operator.itemgetter(conf_pred)) 
        ...
        #     ## done adddd
            ...
            # y_pred = np.array(y_pred)
            # # print('------------------------------y_pred filtered--------------------------------: ', y_pred[0][0])
            ...
            # xet tung hinh anh tren tung batch_size (duyet tung array trong list y_pred)
            for k, batch_item in enumerate(y_pred):
                # lay id (nhan) cua hinh anh
                image_id = batch_image_ids[k]

                # duyet tung box co tren hinh anh image_id
                # moi array co 1 list box (predicted bouding boxes)
        ...
        self.prediction_results = results # tra ve cac bounding box co confidence > 0, la 1 list, moi phan tu tuong ung voi cac confidences cua 1 class_id
        # for i in range(len(self.prediction_results)): 
        #     count = 0
        #     print('predict result 0: ', self.prediction_results[i])
        #     print('type: ', type(self.prediction_results[i]))
        #     print('len: ', len(self.prediction_results[i])) 
        # print('self prediction_resultsssssssssssssss: ', len(self.prediction_results))
        # print('len bouding boxes: ', count)

        print('self.prediction results: ', self.prediction_results)
    ...
    def get_num_gt_per_class(self,
        ...
        class_id_index = self.gt_format['class_id'] # lay chi so class_id trong dinh dang gt_format (hien tai la 0)

        ground_truth = self.data_generator.labels
        # count = 0
        # for i in range(len(ground_truth)):
        #     count += ground_truth[i].shape[0]
        ...
        print('num gt per class: ', self.num_gt_per_class)
    ...
    def match_predictions(self,
        ...
        # print('ground truthhhhhhhhhhhhhhh: ', ground_truth)

        true_positives = [[]] # The true positives for each class, sorted by descending confidence.
        false_positives = [[]] # The flase positives for each class, sorted by descending confidence.
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            if len(predictions) == 0:
                ...
                print('cumulative_false_pos: ', cumulative_false_pos)
            ...
            predictions_sorted = predictions[descending_indices] # predictions theo thu tu giam dan confidence
            ...
            for i in tr:
                ...
                if ignore_neutral_boxes and eval_neutral_available:
                    ...
                    gt = ground_truth[image_id] # lay GT cua hinh anh image_id
                gt = np.asarray(gt)
                class_mask = gt[:,class_id_gt] == class_id # tim cac GT cua class hien tai (class_id)
                gt = gt[class_mask]
                if ignore_neutral_boxes and eval_neutral_available:
                ...
                # print('gt_match_overlap: ', gt_match_overlap)
                if gt_match_overlap < matching_iou_threshold:
                    ...
                    if not (ignore_neutral_boxes and eval_neutral_available) or (eval_neutral[gt_match_index] == False):
                        ...
                        if not (image_id in gt_matched): # image_id chua co trong gt_matched
                            # True positive:
                        ...
                        elif not gt_matched[image_id][gt_match_index]: # image_id cua gt_match_index chua co trong gt_matched
                            # True positive:
                            # If the matched ground truth box for this prediction hasn't been matched to a
                            # different prediction already, we have a true positive.
                            true_pos[i] = 1
                            gt_matched[image_id][gt_match_index] = True
                        # else: # neu da ton tai truoc do => xac dinh la false pos
        ...
        print('self.cumulative_true_positives: ', len(self.cumulative_true_positives))
        print('self.cumulative_false_positives: ', len(self.cumulative_false_positives))
    ...
    def compute_precision_recall(self, verbose=True, ret=False):
        ...
        for class_id in range(1, self.n_classes + 1): 
            ...
            cumulative_precision = np.where(tp + fp > 0, tp / (tp + fp), 0) # 1D array with shape `(num_predictions,)` # neu tp + fp > 0 tra ve tp / (tp + fp), nguoc lai tra ve 0
...
# arr = [0 ,1 ,8, 8, 2, 5 ,4 ,2, 3, 3, 3, 5, 2, 5, 8, 3, 2, 1, 2, 3, 0, 1, 1, 1, 1, 1, 2, 1, 1, 2, 1, 1, 0, 1, 1, 1, 1,
# 1, 0, 8, 1, 2, 1, 1]
# print(len(arr))
# # 0 
# #  20
